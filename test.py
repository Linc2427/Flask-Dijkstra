...
def dijkstra(graph, start, end):
    ...
    while True:
        ...
        for neighbor in graph[closest_node]:
            if neighbor in distances:  # Check if neighbor exists in distances
                distance = distances[closest_node] + haversine_distance(graph[closest_node], graph[neighbor])
                if distance < distances[neighbor]:
                    distances[neighbor] = distance

        del distances[closest_node]  # Remove the closest_node from distances
    ...
    while True:
        ...
        if closest_node == start:
            break
        closest_node_neighbors = graph.get(closest_node, [])
        if closest_node_neighbors:
            closest_node = min(closest_node_neighbors, key=lambda x: distances.get(x, float('inf')))
...
# Update your index function accordingly
...
def index():
    if request.method == 'POST':
        ...
        # Asumsikan input pengguna sebagai titik awal dan akhir
        start = (cur_lat, cur_long)
        end = (veh_lat, veh_long)

        # Temukan titik terdekat dari input pengguna
        closest_start = min(graph, key=lambda x: haversine_distance(start, graph[x]))
        closest_end = min(graph, key=lambda x: haversine_distance(end, graph[x]))

        # Hitung rute terpendek
        shortest_path = dijkstra(graph, closest_start, closest_end)
